...
class ServiceCall {
    ...
    class func post(parameter: NSDictionary, path: String, isToken: Bool = false, withSuccess: @escaping ((_ responseObj: AnyObject?) ->() ), failure: @escaping ((_ error: Error) ->() )) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            let parameters = "email=jason%40gmail.com&password=password"
            ...
            for dictKey in dictKey {
                if let values = parameter.value(forKey: dictKey) as? String {
                    parameterData.append(String.init(format: "%@%@=%@", i == 0 ? "" : "&", dictKey,
                                                     values.replacingOccurrences(of: "+", with: "%2B")).data(using: .utf8)!)
                } else {
                    parameterData.append(String.init(format: "%@%@=%@", i == 0 ? "" : "&", dictKey,
                                                     parameter.value(forKey: dictKey) as! CVarArg).data(using: .utf8)!)
                }
            ...
            let task = URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                if let error = error {
                    ...
                    if let data = data {
                        do {
                            let jsonDictionary = try JSONSerialization.jsonObject(with: data, options: .mutableContainers) as?
                            NSDictionary
